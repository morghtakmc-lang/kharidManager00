...
public class MainActivity extends Activity {
    ...
    void form(JSONObject old) {
        ...
        inputs.get(8).addTextChangedListener(calcW);
        inputs.get(9).addTextChangedListener(calcW);
        ...
        if (old != null) {
            ...
            time.setText(
                    old.optBoolean("alarm", false)
                            ? old.optString("alarmTime", "")
                            : ""
            );

            repeat.setSelection(
                    old.optBoolean("alarmRepeat", false)
                            ? 1
                            : 0
            );
        }
    ...
    void setupValidationListeners(Button save) {
        ...
        if (buyerSp != null)
            buyerSp.setOnItemSelectedListener(l);

        if (commoditySp != null)
            commoditySp.setOnItemSelectedListener(l);

        if (paymentSp != null)
            paymentSp.setOnItemSelectedListener(l);

        if (companySp != null)
            companySp.setOnItemSelectedListener(l);

        for (int i :
                new int[]{2, 3, 4, 7, 8, 9, 11}) {

            inputs.get(i).addTextChangedListener(
                    new TextWatcher() {
    ...
    boolean validateForm(Button save) {
        ...
        ok &= markSpinner(
                buyerSp,
                buyerSp != null &&
                        buyerSp.getSelectedItemPosition() > 0
        );

        ok &= markSpinner(
                commoditySp,
                commoditySp != null &&
                        commoditySp.getSelectedItemPosition() > 0
        );

        ok &= markSpinner(
                paymentSp,
                paymentSp != null &&
                        paymentSp.getSelectedItemPosition() > 0
        );

        ok &= markSpinner(
                companySp,
                companySp != null &&
                        companySp.getSelectedItemPosition() > 0
        );

        int[] req = {2, 3, 4, 7, 8, 9, 11};
        ...
        if (nums) {
            ...
            try {
                ...
                long so = Long.parseLong(ss);

                boolean match = (c + so) == w;

                markField(inputs.get(3), match);
                markField(inputs.get(8), match);
                markField(inputs.get(9), match);
                ...
                markField(inputs.get(3), false);
                markField(inputs.get(8), false);
                markField(inputs.get(9), false);
            }
    ...
    boolean markSpinner(
        ...
        if (s != null)
            s.setBackground(fieldBg(valid));

        return valid;
    }

    GradientDrawable fieldBg(boolean valid) {

        GradientDrawable g =
                new GradientDrawable();

        g.setColor(UiManager.card(this));

        g.setCornerRadius(
                UiManager.dp(
                        this,
                        UiManager.radius(this)
                )
        );
    ...
    void updateAlarmStatus(
        ...
        if (d.isEmpty() && t.isEmpty()) {
            v.setText("هشدار تنظیم نشده است");
            return;
        }

        if (!d.matches("\\d+") ||
                !t.matches("([01]\\d|2[0-3]):[0-5]\\d")) {

            v.setText("⚠ تنظیم هشدار کامل نیست");
            return;
        }
    ...
    void savePurchase(
        ...
        if (!validateForm(saveButton)) {

            Toast.makeText(
                    this,
                    "اطلاعات ناقص یا مقدار ذرت و سویا با وزن برابر نیست",
                    Toast.LENGTH_LONG
            ).show();
    ...
    void listPurchases(
        ...
        Button f1 = btn("وصول شده");
        Button f2 = btn("وصول نشده");
        Button f3 = btn("تخصیص شده");
        Button f4 = btn("تخصیص نشده");
        ...
        filterRow.addView(r1);
        filterRow.addView(r2);
    ...
    void buyersPage() {
        ...
        base("خریداران و پرونده هر خریدار");
    ...
    void buyerFile(String buyer) {
        ...
        ArrayList<JSONObject> noColl =
                new ArrayList<>();

        ArrayList<JSONObject> noAlloc =
                new ArrayList<>();
        ...
        addPurchaseNumberList(noAlloc);

        add(
                tv(
                        "فیلتر سریع خریدهای این خریدار",
                        16
                )
        );
    ...
    void styleReportCheck(CheckBox c) {
