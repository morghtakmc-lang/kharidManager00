...
public class MainActivity extends Activity {
    ...
    void form(JSONObject old){
        ...
        Button collected=toggleButton("وصول",old!=null&&old.optBoolean("collected",false)); add(collected);
        Button allocated=toggleButton("تخصیص",old!=null&&old.optBoolean("allocated",false)); add(allocated);

        add(tv("هشدار سررسید اصلی",16));
        Button alarm=toggleButton("هشدار",old!=null&&old.optBoolean("alarm",false)); add(alarm);
        ...
        if(old!=null){
            days.setText(""+old.optInt("alarmDays",1));time.setText(old.optString("alarmTime","10:00"));
            repeat.setSelection(old.optBoolean("alarmRepeat",false)?1:0);
            updateAlarmPreview(preview,old.optString("mainDue"),days.getText().toString(),time.getText().toString(),alarm.isSelected());
        }
        TextWatcher aw=new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){updateAlarmPreview(preview,inputs.get(11).getText().toString(),days.getText().toString(),time.getText().toString(),alarm.isSelected());}public void afterTextChanged(Editable e){}};
        days.addTextChangedListener(aw);time.addTextChangedListener(aw);inputs.get(11).addTextChangedListener(aw);
        alarm.setOnClickListener(v->{alarm.setSelected(!alarm.isSelected());styleToggle(alarm);updateAlarmPreview(preview,inputs.get(11).getText().toString(),days.getText().toString(),time.getText().toString(),alarm.isSelected());});
        collected.setOnClickListener(v->{collected.setSelected(!collected.isSelected());styleToggle(collected);});
        allocated.setOnClickListener(v->{allocated.setSelected(!allocated.isSelected());styleToggle(allocated);});

        Button clear=btn("🔕 حذف هشدار");clear.setOnClickListener(v->{alarm.setSelected(false);styleToggle(alarm);if(old!=null){try{old.remove("alarm");old.remove("alarmRepeat");}catch(Exception ignored){}cancelAlarm(this,old);}});
        add(clear);
        Button save=btn("✓ ذخیره خرید و هشدار");
        save.setOnClickListener(v->savePurchase(old,collected.isSelected(),allocated.isSelected(),alarm.isSelected(),days.getText().toString(),time.getText().toString(),repeat.getSelectedItemPosition()==1));
        add(save);
    ...
    Button toggleButton(String label,boolean on){
        Button b=btn(on?label+" شد":label);b.setSelected(on);styleToggle(b);return b;
    }
    void styleToggle(Button b){
        boolean on=b.isSelected();
        b.setText(on?(baseToggleText(b.getText().toString())+" شد"):baseToggleText(b.getText().toString()));
        b.setTextColor(on?Color.WHITE:UiManager.text(this));
        android.graphics.drawable.GradientDrawable bg=new android.graphics.drawable.GradientDrawable();
        bg.setColor(on?UiManager.primary(this):UiManager.secondary(this));
        bg.setCornerRadius(UiManager.dp(this,14));
        bg.setStroke(UiManager.dp(this,1),on?UiManager.primary(this):Color.LTGRAY);
        b.setBackground(bg);
        b.setAllCaps(false);
        b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        b.setGravity(Gravity.CENTER);
    }
    String baseToggleText(String s){return s.replace(" شد","").replace("✓ ","").trim();}
    ...
    void listPurchases(String fixedBuyer){
        base(fixedBuyer==null?"خریدهای ثبت‌شده":"پرونده "+fixedBuyer);EditText q=input("جستجو: نام، شماره خرید، شرکت یا نهاده");
        LinearLayout ds=new LinearLayout(this);EditText from=new EditText(this),to=new EditText(this);from.setHint("از تاریخ");to.setHint("تا تاریخ");from.setOnClickListener(v->pickDate(from));to.setOnClickListener(v->pickDate(to));
        ds.addView(from,new LinearLayout.LayoutParams(0,60,1));ds.addView(to,new LinearLayout.LayoutParams(0,60,1));add(ds);Button go=btn("🔎 جستجو / فیلتر");add(go);LinearLayout list=new LinearLayout(this);list.setOrientation(LinearLayout.VERTICAL);add(list);
    ...
    void reportSelection(String title,String filterKey,String filterVal){
        ...
        for(int i=0;i<a.length();i++){JSONObject p=a.optJSONObject(i);if(!filterVal.equals(p.optString(filterKey)))continue;CheckBox c=new CheckBox(this);c.setText("خرید "+p.optString("purchaseNo")+" | "+p.optString("buyDate")+" | "+AppData.fmt(p.optString("amount")));checks.add(c);objs.add(p);list.addView(c);}
        ...
        Button all=btn("☑ انتخاب همه موارد نمایش‌داده‌شده");all.setOnClickListener(v->{for(CheckBox c:checks)if(c.getVisibility()==View.VISIBLE)c.setChecked(true);});add(all);
    ...
    class PersianCalendarDialog extends Dialog {
        EditText target; int jy,jm,jd;
        TextView title; LinearLayout grid;
        ...
        @Override protected void onCreate(Bundle b){
            super.onCreate(b); LinearLayout box=new LinearLayout(MainActivity.this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(18,12,18,12);
            title=new TextView(MainActivity.this);title.setGravity(Gravity.CENTER);title.setTextSize(18);box.addView(title,new LinearLayout.LayoutParams(-1,60));
            ...
            nav.addView(prevYear,new LinearLayout.LayoutParams(0,58,1));
            nav.addView(prev,new LinearLayout.LayoutParams(0,58,1));
            nav.addView(next,new LinearLayout.LayoutParams(0,58,1));
            nav.addView(nextYear,new LinearLayout.LayoutParams(0,58,1));
            box.addView(nav);
            grid=new LinearLayout(MainActivity.this);grid.setOrientation(LinearLayout.VERTICAL);box.addView(grid);
            LinearLayout bottom=new LinearLayout(MainActivity.this);Button cancel=btn("لغو"),ok=btn("تأیید");bottom.addView(cancel,new LinearLayout.LayoutParams(0,58,1));bottom.addView(ok,new LinearLayout.LayoutParams(0,58,1));box.addView(bottom);
            ...
            setContentView(box);Window w=getWindow();if(w!=null)w.setLayout(-1,-2);render();
        }
        int monthDays(int y,int m){return m<=6?31:(m<=11?30:(isLeap(y)?30:29));}
        boolean isLeap(int y){int[] g=PersianDate.toGregorian(y+1,1,1);long a=PersianDate.millis(String.format(Locale.US,"%04d/%02d/%02d",y,12,1),"00:00");long z=PersianDate.millis(String.format(Locale.US,"%04d/%02d/%02d",y+1,1,1),"00:00");return z-a>365L*86400000L;}
        void render(){
            title.setText("تقویم شمسی  "+jy+" / "+monthName(jm));
            grid.removeAllViews();String[] wd={"ش","ی","د","س","چ","پ","ج"};
            LinearLayout heads=new LinearLayout(MainActivity.this);for(String s:wd){TextView x=tv(s,14);x.setGravity(Gravity.CENTER);heads.addView(x,new LinearLayout.LayoutParams(0,45,1));}grid.addView(heads);
            ...
            for(int row=0;row<6&&day<=days;row++){LinearLayout r=new LinearLayout(MainActivity.this);for(int col=0;col<7;col++){Button d=btn("");d.setTextColor(UiManager.text(MainActivity.this));if((row==0&&col<first)||day>days)d.setEnabled(false);else{final int dd=day;d.setText(""+day);if(day==jd){d.setSelected(true);d.setBackgroundColor(UiManager.primary(MainActivity.this));d.setTextColor(Color.WHITE);}d.setOnClickListener(v->{jd=dd;render();});day++;}r.addView(d,new LinearLayout.LayoutParams(0,52,1));}grid.addView(r);}
        }
