...
public class MainActivity extends Activity {
    ...
    void form(JSONObject old){
        ...
        updateAlarmStatus(preview,days.getText().toString(),time.getText().toString(),inputs.get(13).getText().toString());
    ...
    void listPurchases(String fixedBuyer){
        ...
        Runnable render=()->{
            list.removeAllViews();int count=0,col=0,alloc=0,full=0;long amount=0,funded=0;
            ...
            for(int i=0;i<a.length();i++){
                ...
                count++;amount+=toLong(p.optString("amount"));funded+=totalFunded(p);
                ...
                Button item=btn("👤 "+p.optString("buyer")+" | خرید "+p.optString("purchaseNo")+
                        ...
                        " | "+(complete?"تأمین: کامل":"تأمین: ناقص"));
    ...
    void details(JSONObject p){
        ...
        if(p.optBoolean("alarm"))add(tv("🔔 هشدار مستقل: "+p.optInt("alarmDays",1)+" روز قبل، ساعت "+
                p.optString("alarmTime")+" | "+(p.optBoolean("alarmRepeat")?"تکراری":"یک‌بار"),15));
    ...
    void fundingDialog(JSONObject p){
        ...
        EditText source=new EditText(this);source.setHint("منبع تأمین");box.addView(source);
        EditText dest=new EditText(this);dest.setHint("مقصد / حساب");box.addView(dest);
        ...
        new AlertDialog.Builder(this).setTitle("ثبت مرحله تأمین موجودی").setView(box)
            .setPositiveButton("ثبت مرحله",(d,w)->{
                try{
                    ...
                    st.put("source",source.getText().toString().trim());st.put("destination",dest.getText().toString().trim());arr.put(st);
