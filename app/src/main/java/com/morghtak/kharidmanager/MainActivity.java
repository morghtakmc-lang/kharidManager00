...
public class MainActivity extends Activity {
    ...
    void unitGroupPage(boolean chain){
        ...
        Runnable render=()->{
            ...
            for(int i=0;i<group.length();i++){
                ...
                row.addView(x,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f));
                Button edit=btn("✏️");
                edit.setOnClickListener(v->editUnitName(name,chain));
                row.addView(edit,new LinearLayout.LayoutParams(UiManager.dp(this,58),ViewGroup.LayoutParams.WRAP_CONTENT));
                list.addView(row);visible++;
            }
    ...
    void addGroupedUnit(boolean toChain){
        ...
        new AlertDialog.Builder(this).setTitle(toChain?"افزودن واحد زیرمجموعه به زنجیره":"افزودن واحد مستقل").setView(e)
            .setPositiveButton("ذخیره",(d,w)->{try{String v=e.getText().toString().trim();if(v.isEmpty())return;if(ZANJIREH_UNIT.equals(v)){Toast.makeText(this,"نام واحد اصلی از قبل وجود دارد.",Toast.LENGTH_LONG).show();return;}if(SHAHEDANEH_UNIT.equals(v)){Toast.makeText(this,"شاهدانه طیور مارلیک از قبل وجود دارد و منطق مستقل خودش را دارد.",Toast.LENGTH_LONG).show();return;}JSONArray chain=unitGroup("zanjireh"),ind=unitGroup("independent");if(containsJsonString(chain,v)||containsJsonString(ind,v)){Toast.makeText(this,"این واحد قبلاً ثبت شده است.",Toast.LENGTH_LONG).show();return;} (toChain?chain:ind).put(v);JSONArray units=AppData.arr(data,"units");if(!containsJsonString(units,v))units.put(v);data.put("units",units);AppData.save(this,data);unitGroupPage(toChain);}catch(Exception ignored){}})
            .setNegativeButton("لغو",null).show();
    }

    void editUnitName(final String oldName, final boolean inChain){
        EditText e=new EditText(this);e.setSingleLine(true);e.setText(oldName);e.setSelectAllOnFocus(true);
        new AlertDialog.Builder(this).setTitle("ویرایش نام واحد").setView(e)
            .setPositiveButton("ذخیره",(d,w)->{
                String newName=e.getText().toString().trim();
                if(newName.isEmpty()||newName.equals(oldName))return;
                if(ZANJIREH_UNIT.equals(oldName)||SHAHEDANEH_UNIT.equals(oldName)){
                    Toast.makeText(this,"نام این واحد ثابت است.",Toast.LENGTH_LONG).show();return;
                }
                try{
                    JSONArray chain=unitGroup("zanjireh"),ind=unitGroup("independent"),units=AppData.arr(data,"units");
                    if(containsJsonString(chain,newName)||containsJsonString(ind,newName)||containsJsonString(units,newName)){
                        Toast.makeText(this,"این نام قبلاً برای یک واحد ثبت شده است.",Toast.LENGTH_LONG).show();return;
                    }
                    JSONArray group=inChain?chain:ind;
                    for(int i=0;i<group.length();i++)if(oldName.equals(group.optString(i)))group.put(i,newName);
                    for(int i=0;i<units.length();i++)if(oldName.equals(units.optString(i)))units.put(i,newName);
                    JSONArray purchases=AppData.arr(data,"purchases");
                    for(int i=0;i<purchases.length();i++){JSONObject p=purchases.optJSONObject(i);if(p==null)continue;if(oldName.equals(p.optString("unit")))p.put("unit",newName);if(oldName.equals(p.optString("buyer")))p.put("buyer",newName);if(oldName.equals(p.optString("transferSourceUnit")))p.put("transferSourceUnit",newName);}
                    JSONArray transfers=AppData.arr(data,"quotaTransfers");
                    for(int i=0;i<transfers.length();i++){JSONObject x=transfers.optJSONObject(i);if(x==null)continue;if(oldName.equals(x.optString("unit")))x.put("unit",newName);if(oldName.equals(x.optString("transferSourceUnit")))x.put("transferSourceUnit",newName);}
                    AppData.save(this,data);
                    unitGroupPage(inChain);
                }catch(Exception ex){Toast.makeText(this,"ویرایش نام واحد انجام نشد.",Toast.LENGTH_LONG).show();}
            })
            .setNegativeButton("لغو",null).show();
    }
