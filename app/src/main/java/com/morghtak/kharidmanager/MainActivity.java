...
public final class UiManager {
    ...
    static GradientDrawable bg(int color,float radiusDp,int strokeColor,boolean stroke){
        GradientDrawable g=new GradientDrawable();
        g.setColor(color);
        g.setCornerRadius(radiusDp*3f);
        if(stroke)g.setStroke(1,strokeColor);
        return g;
    }
    ...
    public static void styleText(Context c,TextView v){
        ...
        if(v instanceof Button){
            ...
            v.setPadding(dp(c,14),dp(c,8),dp(c,14),dp(c,8));
    ...
    public static void decorate(final Activity a,final LinearLayout root,final String title){
        ...
        root.setPadding(dp(c,10),dp(c,8),dp(c,10),dp(c,18));
        ...
        if(root.getChildCount()>0&&root.getChildAt(0) instanceof TextView){
            ...
            h.setPadding(dp(c,16),dp(c,8),dp(c,16),dp(c,8));
            h.setBackground(bg(primary(c),radius(c),primary(c),false));
        ...
        if("مدیریت خرید و سررسید".equals(title)){
            ...
            logo.setPadding(0,dp(c,6),0,dp(c,6));
            ...
            root.addView(settings,new LinearLayout.LayoutParams(-1,dp(c,58)));
        }
    ...
    public static void showSettings(final Activity a){
        ...
        new AlertDialog.Builder(a)
            .setTitle("دسترسی مدیر")
            .setMessage("این بخش مخصوص مدیر است.")
            .setView(pin)
    ...
    private static void openSettingsPanel(final Activity a){
        ...
        box.setPadding(dp(a,12),dp(a,4),dp(a,12),dp(a,12));

        ScrollView sv=new ScrollView(a);
        sv.addView(box);

        addTitle(box,a,"تنظیمات ظاهری و مدیریتی");
        addInfo(box,a,"تغییرات را ابتدا در پیش‌نمایش ببینید؛ ذخیره اعمال می‌کند و لغو هیچ تغییری نمی‌دهد.");

        addInfo(box,a,"نام شرکت (فقط نمونه پیش‌نمایش)");
        final EditText sampleName=edit(a,"مثال: نام شرکت نمونه");
        sampleName.setText("نام شرکت نمونه");
        box.addView(sampleName,lp());

        addInfo(box,a,"پیش‌نمایش زنده");

        final LinearLayout preview=new LinearLayout(a);
        preview.setOrientation(LinearLayout.VERTICAL);
        preview.setPadding(dp(a,12),dp(a,12),dp(a,12),dp(a,12));
        ...
        final Spinner font=spinner(a,fontNames);
        selectFont(font,fontValues,font(a));
        box.addView(label(a,"نوع فونت"),lp());
        box.addView(font,lp());

        final SeekBar body=seek(a,11,26,(int)bodySize(a));
        TextView bodyL=label(a,"اندازه متن: "+(int)bodySize(a));
        box.addView(bodyL,lp());
        box.addView(body,lp());

        final SeekBar title=seek(a,17,32,(int)titleSize(a));
        TextView titleL=label(a,"اندازه عنوان: "+(int)titleSize(a));
        box.addView(titleL,lp());
        box.addView(title,lp());

        final SeekBar button=seek(a,12,23,(int)buttonSize(a));
        TextView buttonL=label(a,"اندازه نوشته دکمه: "+(int)buttonSize(a));
        box.addView(buttonL,lp());
        box.addView(button,lp());

        final SeekBar field=seek(a,12,23,(int)fieldSize(a));
        TextView fieldL=label(a,"اندازه نوشته فیلد: "+(int)fieldSize(a));
        box.addView(fieldL,lp());
        box.addView(field,lp());

        final SeekBar radius=seek(a,0,32,(int)radius(a));
        TextView radiusL=label(a,"گردی گوشه‌ها: "+(int)radius(a));
        box.addView(radiusL,lp());
        box.addView(radius,lp());

        final SeekBar spacing=seek(a,4,24,(int)spacing(a));
        TextView spacingL=label(a,"فاصله عناصر: "+(int)spacing(a));
        box.addView(spacingL,lp());
        box.addView(spacing,lp());
        ...
        addPalette(box,a,"رنگ اصلی",tempPrimary,refreshRef);
        addPalette(box,a,"رنگ پس‌زمینه",tempBackground,refreshRef);
        addPalette(box,a,"رنگ کارت/فیلد",tempCard,refreshRef);
        addPalette(box,a,"رنگ متن",tempText,refreshRef);
        ...
        final boolean[] resetLogoRequested={false};

        Button resetLogo=button(a,"↺ بازگردانی لوگوی اصلی شرکت");
        resetLogo.setOnClickListener(v->{
            resetLogoRequested[0]=true;
            Toast.makeText(a,"بازگردانی لوگو برای زمان ذخیره انتخاب شد",Toast.LENGTH_SHORT).show();
        });
        box.addView(resetLogo,lp());

        final EditText newPin=edit(a,"رمز جدید مدیر (خالی = بدون تغییر)");
        newPin.setInputType(2|0x00000010);
        box.addView(newPin,lp());

        Button save=button(a,"✓ ذخیره و اعمال تنظیمات");
        Button cancel=button(a,"لغو");
        box.addView(save,lp());
        box.addView(cancel,lp());

        final AlertDialog dlg=new AlertDialog.Builder(a)
            .setTitle("تنظیمات مدیر")
            .setView(sv)
            .create();
        ...
        final Runnable refreshPreview=()->{
            ...
            boolean isDark=darkBox.isChecked();
            int previewBg=isDark?Color.rgb(30,30,30):tempBackground[0];
            int previewText=isDark?Color.WHITE:tempText[0];
            int previewCard=isDark?Color.rgb(45,45,45):tempCard[0];

            String selectedFont=fontValues[Math.max(0,Math.min(font.getSelectedItemPosition(),fontValues.length-1))];

            preview.setBackgroundColor(previewBg);

            TextView h=label(a,sampleName.getText().toString());
            h.setTextSize(title.getProgress()+17);
            h.setTypeface(Typeface.create(selectedFont,Typeface.BOLD));
            h.setTextColor(Color.WHITE);
            h.setGravity(Gravity.CENTER);
            h.setBackground(bg(tempPrimary[0],radius.getProgress()+0f,tempPrimary[0],false));
            h.setPadding(dp(a,8),dp(a,8),dp(a,8),dp(a,8));
            preview.addView(h,lp());

            TextView bodyV=label(a,"این متن نمونه برای مشاهده تغییر فونت، اندازه و رنگ است.\nAa 12345");
            bodyV.setTextSize(body.getProgress()+11);
            bodyV.setTypeface(Typeface.create(selectedFont,Typeface.NORMAL));
            bodyV.setTextColor(previewText);
            preview.addView(bodyV,lp());

            Button bv=button(a,"دکمه نمونه");
            bv.setTextSize(button.getProgress()+12);
            bv.setTypeface(Typeface.create(selectedFont,Typeface.BOLD));
            bv.setBackground(bg(tempPrimary[0],radius.getProgress()+0f,tempPrimary[0],false));
            preview.addView(bv,lp());

            EditText ev=edit(a,"نمونه فیلد");
            ev.setTextSize(field.getProgress()+12);
            ev.setTypeface(Typeface.create(selectedFont,Typeface.NORMAL));
            ev.setTextColor(previewText);
            ev.setHintTextColor(isDark?Color.rgb(190,190,190):textSecondary(a));
            ev.setBackground(bg(previewCard,radius.getProgress()+0f,tempPrimary[0],true));
            preview.addView(ev,lp());
        };
        ...
        SeekBar.OnSeekBarChangeListener listener=new SeekBar.OnSeekBarChangeListener(){
            public void onProgressChanged(SeekBar s,int pos,boolean fromUser){
                bodyL.setText("اندازه متن: "+(body==s?pos+11:body.getProgress()+11));
                titleL.setText("اندازه عنوان: "+(title==s?pos+17:title.getProgress()+17));
                buttonL.setText("اندازه نوشته دکمه: "+(button==s?pos+12:button.getProgress()+12));
                fieldL.setText("اندازه نوشته فیلد: "+(field==s?pos+12:field.getProgress()+12));
                radiusL.setText("گردی گوشه‌ها: "+radius.getProgress());
                spacingL.setText("فاصله عناصر: "+(spacing.getProgress()+4));
                refreshPreview.run();
            }
        ...
        body.setOnSeekBarChangeListener(listener);
        title.setOnSeekBarChangeListener(listener);
        button.setOnSeekBarChangeListener(listener);
        field.setOnSeekBarChangeListener(listener);
        radius.setOnSeekBarChangeListener(listener);
        spacing.setOnSeekBarChangeListener(listener);
        ...
        sampleName.addTextChangedListener(new TextWatcher(){
        ...
        save.setOnClickListener(v->{
            ...
            int fontPos=Math.max(0,Math.min(font.getSelectedItemPosition(),fontValues.length-1));

            e.putString("font",fontValues[fontPos]);
            e.putFloat("body",body.getProgress()+11);
            e.putFloat("title",title.getProgress()+17);
            e.putFloat("button",button.getProgress()+12);
            e.putFloat("field",field.getProgress()+12);
            e.putFloat("radius",radius.getProgress());
            e.putFloat("spacing",spacing.getProgress()+4);
    ...
    private static void addPalette(LinearLayout box,Activity a,String title,int[] holder,final Runnable[] refreshRef){
        ...
        for(int col:colors){
            ...
            b.setOnClickListener(v->{
                ...
                Toast.makeText(a,title+" انتخاب شد",Toast.LENGTH_SHORT).show();
            });

            row.addView(b,new LinearLayout.LayoutParams(0,46,1));
        }
    ...
    public static void handleLogoResult(Activity a,int requestCode,int resultCode,Intent data){
        ...
        try{
            ...
            Toast.makeText(a,"خطا در انتخاب لوگو",Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private static void addTitle(LinearLayout b,Activity a,String s){
        TextView t=label(a,s);
        t.setTextSize(titleSize(a));
        t.setTypeface(selectedTypeface(a,Typeface.BOLD));
        b.addView(t,lp());
    }

    private static void addInfo(LinearLayout b,Activity a,String s){
        b.addView(label(a,s),lp());
    }

    private static LinearLayout.LayoutParams lp(){
        return new LinearLayout.LayoutParams(-1,ViewGroup.LayoutParams.WRAP_CONTENT);
    }

    private static Spinner spinner(Activity a,String[] values){
        Spinner s=new Spinner(a);
        s.setAdapter(new ArrayAdapter<String>(a,android.R.layout.simple_spinner_dropdown_item,values));
        return s;
    }

    private static void selectFont(Spinner s,String[] values,String selected){
    ...
    private static SeekBar seek(Activity a,int min,int max,int current){
        SeekBar s=new SeekBar(a);
        s.setMax(max-min);
        s.setProgress(Math.max(0,Math.min(max-min,current-min)));
        return s;
    }
}
